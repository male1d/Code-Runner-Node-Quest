    ...
    namespace Game
    {
        ...
        private void LoadLevel()
        {
            try
            {
                ...
                for (int i = 0; i < grid.GetLength(0); i++) // Инициализация массива
                {
                    ...
                    {
                        grid[i, j] = null; // Изначально пусто
                    }
                }
                for (int y = 0; y < rows.Length; y++)
                {
                    ...
                    for (int x = 0; x < chars.Length; x++)
                    {
                        ...
                        switch (chars[x])
                        {
                            ...
                            case '@':
                                dim = new(size, size / 2);
                                grid[x, y] = new(GetPosition(x, y) + dim / 2 + new Vector(0, size / 2), dim, $"{prefix}img/block/2222.jpg", true);
                                break;
                            case '[':
        ...
        private void HandleCollisions(Vector old)
        {
            ...
            foreach (var block in GetAdjacent(x, y))
            {
                if (block != null && block.isSolid && player.IsIntersecting(block))
                {
                    ...
                    {
                        prompt.text = "[ПРОБЕЛ] Следующий уровень";
                        nearFinish = true;
                    }
                    else
                    {
        ...
        private bool ActivateNode(Node node)
        {
            return node.children.Length switch
            {
                ...
                _ => throw new Exception("Узловая контрацепция дала сбой."),
            };
        }
        ...
        private void DrawWorld(Graphics g)
        {
            if (cam.pos.y + view.y / 2 > 0)
            {
                ...
                g.FillRectangle(brush, 0, y, view.x, view.y - y); // бесконечный этаж
            }
            ...
            foreach (var node in circuit.nodes)
            {
                ...
                if (node.gateName == null)
                {
                    ...
                    {
                        prompt.text = "[ПРОБЕЛ] Переключить";
                    }
                }
                else
                {
